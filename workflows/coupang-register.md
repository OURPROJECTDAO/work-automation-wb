...
- ★★ **관리코드가 날짜로 해석된다** (2026-08-27). `45-12-05` 3건이 판매자상품코드 **`2045-12-05`** 로
  등록됐다. `NN-MM-DD` 가 월·일 범위에 들면 연도 접두가 붙는다(같은 배치의 50-71-06·50-75-05·50-71-05 는 무사).
  → cmm '관리코드 미등록'으로 마진 계산 불가. **등록 후 판매자상품코드를 눈으로 확인**하고,
  걸리면 WING 에서 수기 정정. hapo 는 옵션ID 기준이라 코드 오기와 무관하게 먼저 넣어도 된다.
- ★★ **CSV 한 행 = sellerProduct 1개** (2026-08-26 실증). `coupang_batch.py` 는 행마다 별도 상품을
  생성하므로, 같은 관리코드를 여러 행으로 넣으면 **옵션으로 묶이지 않고 상품이 쪼개진다**
  (9행 → 9개 상품). 위 "한 상품에 1~박스내품 세트를 옵션으로" 규칙과 어긋난다.
  → 현재는 **등록 후 WING에서 수동 병합**. 다음 개선 = 스크립트를 `code` 기준 그룹핑해
  `items` 배열로 한 번에 생성. ★병합하면 **옵션ID가 바뀌므로 hapo 는 병합 후에 등록**할 것.
- ★ **수수료 가정은 12%로** (2026-08-26 사용자 확정). 카테고리 판매대행수수료(9.6~10.6%)로 계산하면
  cmm(쿠팡 12% 단일) 화면에서 실현 마진이 3.5~4.0% 로 떨어진다. 12% 로 잡아야 기준마진과 맞는다.
- **비식품 선물세트 카테고리** (2026-08-26): 샴푸·바디워시가 섞인 종합 선물세트는 생활용품 트리에
  자리가 없다(`생활용품.xlsx` 에 샴푸/바디워시 카테고리 자체가 없음). **뷰티 참조표의 `뷰티>선물세트`**
  전용 트리를 쓴다 — **111240 멀티케어선물세트**(복합) · 111238 바디선물세트 · 111234 스킨 ·
  111237 헤어 · 111235 메이크업 · 111236 향수 · 111239 남성화장품. 전부 수수료 9.6%.
  실적: 45-20-05 애경 마음14호 → **111240**, `수량` 옵션만으로 1회 통과.
- 배치 실적: **선물세트 신규 2종 9옵션(2026-08-26)** — 45-28 CJ스팸복합100호(1~3세트·59882) ·
  45-20-05 애경 마음14호(1~6세트·111240). 마진 5%·수수료 12%. **9/9 성공**, hapo 7행 커밋(b51c7505).
  ⚠️ 상품 9개로 쪼개져 수동 병합 대기.
...
_갱신: 2026-08-26 (★CSV 행 단위 = 상품 1개(옵션 안 묶임, 수동 병합)·수수료 가정 12%로 정정·비식품 선물세트는 뷰티>선물세트 트리(111240 등). 선물세트 2종 9옵션 등록 성공 + hapo 7행)_

_갱신: 2026-08-27 (★관리코드 날짜 변환 함정 `45-12-05`→`2045-12-05` · 선물세트 4종 14옵션 등록·hapo 10행 `9fc494f3`)_

...
- ✅ **상품마진(온라인) 탭**(2026-06-10, ADR 0012): 온라인 거래처 = **store_groups '온라인' 그룹**(15개; 00-12 자동판별 폐기 — 오프라인 오포함 때문, 2026-06-10 fix d5a5b1b2). 추정송장=수량÷(합포수량×박스내품), 추정택배=실택배(3000/2500)×추정송장×**보정계수 k**(=실제송장÷추정송장, 기간·거래처별 자동). 마진율=(판매이익−추정택배)÷(판매금액−판매이익). 보정 시 채널 실제값에 수렴(스마트스토어 5월 5.98%), 미보정 낙관(6.89%). load_sales에 합포수량·박스내품 컬럼. page-only(Reboot 불필요).

...
- 기준데이터: `reference/logistics_classification.csv`(구분), `reference/product_master.csv`(중분류·박스내품), **`reference/product_attributes.csv`(관리코드 속성표 1,134 — 식품음료·합포수량·최종분류 4컬럼; ADR 0011, 합포데이터.xlsx에서 추출)**.
...
- ✅ **상품마진(온라인) 탭**(2026-06-10, ADR 0012): 온라인 거래처 = **store_groups '온라인' 그룹**(15개; 00-12 자동판별 폐기 — 오프라인 오포함 때문, 2026-06-10 fix d5a5b1b2). 추정송장=수량÷(합포수량×박스내품), 추정택배=실택배(3000/2500)×추정송장×**거래처별 보정계수 k**(상호명별 실제송장÷추정송장, 기간별 자동). 집계기준=거래처/관리코드/상품명/세분류(거래처 차원으로 채널 비교). 거래처 선택은 체크박스+전체선택/해제 토글(om_store_bulk/ver). 마진율=(판매이익−추정택배)÷(판매금액−판매이익). 보정 시 채널 실제값에 수렴(스마트스토어 5월 5.98%), 미보정 낙관(6.89%). load_sales에 합포수량·박스내품 컬럼. page-only(Reboot 불필요). **✅ P2 실측 교체(2026-06-15, intelligence-layer §2B): 택배비를 EasyAdmin 송장그룹 실배분(EA박스÷매출낱개 강도)+ (상호명,월) 00-12 정합으로 대체(추정송장·k는 fallback). 토글 `use_actual`(기본 ON·OFF=기존추정). EA 미경유 6채널/2026이전 월=추정 fallback. 컬럼 추정택배→택배비. 캐시 `load_ship_rate`(orders.read_all+load_sales+compute_ship_rate). ★단위=매출낱개≠EA판매단위(배율≈박스내품). ⚠️core ship_alloc import→Reboot. core/intelligence/ship_alloc.py. logs 2026-06-15-ship-alloc-p2.**
...
- logs/2026-06/2026-06-15-ship-alloc-p2.md (온라인마진 택배비 EA 송장 실측+00-12 정합 교체)

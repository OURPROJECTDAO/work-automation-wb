...
선물세트만 따로 보기 위한 **표시·필터 전용** 컬럼. 마진 계산엔 영향 없음.

| 값 | 판정 |
|---|---|
| `시즌라인업` | `reference/giftset_lineup_2026.csv` 멤버십 (**시즌 정본**) |
| `선물세트` | `product_attributes` 최종분류=='선물세트' 인데 라인업 외 |
| (빈칸) | 일반상품 |

라인업 ⊂ 선물세트(2026-08-28 감사로 40/40 확인)라 단일 컬럼 3값으로 충분.
UI = 필터 줄 맨 왼쪽 `세트구분` 셀렉트박스(전체/선물세트만/시즌라인업만/일반상품만).
`_stats["시즌라인업"]` 집계도 추가.

★ **`코드유형`에 섞지 않았다.** 박스/PC낱개/소분/합포는 **매입가 산출 방식**을 나타내는 구조적 축이라
성격이 다른 선물세트를 넣으면 필터가 망가진다.

실측(2026-08-28): ESM 시즌라인업 195 / 라인업외 선물세트 2 · 캐시노트 67 / 4 · 스마트스토어 98 / 1.
**"시즌라인업만 + 기준 초과만"** 조합이 선물세트 가격 정렬의 표준 진입점.

## ★ 채널 의무쿠폰 (channel_coupon.csv) — 2026-08-28

채널이 **의무적으로 발행하게 하는 정률 쿠폰**을 마진·권장가에 반영. 부담이 우리일 때만 등록한다
(식봄위크 쿠폰은 플랫폼 부담이라 미등록 — **채널마다 다르니 매번 확인**).

**수수료는 쿠폰 *전* 판매가에 부과 → 승산이 아니라 감산**(사용자 확정, 불리한 쪽. decisions/0031):
...
- 캐시노트 실효계수 `0.94 − 0.05 = 0.89` (승산 모델 0.8930보다 0.3%p 불리)
- **배송비 항엔 미적용** · 정률 상한 없음
- 권장가는 `rate` 하나만 쓰므로 수식 수정 없이 자동 반영

**파일**: `채널,쿠폰율,적용대상,시작일,종료일,비고`. 채널 키 = **`baseline_col`**(채널 표시명).
`적용대상`은 `product_attributes.최종분류` 값(예 `선물세트`) 또는 `전체`.
시작일·종료일 비우면 무기한. 쿠폰율 0 또는 행 삭제 = 해제. **미정의 채널은 0 → 기존 동작과 동일.**

★ **마진율 컬럼의 의미가 바뀌었다** — `마진율` = 쿠폰 반영 **실질값**, `쿠폰전마진율`이 참고 컬럼.
탐지·하한·권장가가 전부 마진율에 물려 있어 실질로 두지 않으면 대시보드가 과대평가된다.
쿠폰 대상 상품이 무더기로 마진미달로 뜨는 건 오탐이 아니라 드러나지 않던 진실.

★ **마진 하락폭 ≠ 쿠폰율.** 배송비 항 무시 시 `1 − m′ = (1 − m)/(1 − 쿠폰)` →
**마진이 낮을수록 타격이 크고, 기준마진이 쿠폰율과 같으면 정확히 0%가 된다.**
(캐시노트 실측: 평균 10.80% → 6.13%, −4.67%p. 기준 5% 상품들은 0.2~1.4%로 추락.)

⚠️ **편집 즉시 반영 안 됨** — 코어가 로컬 `reference/`를 읽으므로 UI 저장은 커밋만 되고
표 계산은 **배포(1~2분) 후** 반영. baseline/floor처럼 GitHub 라이브 override를 붙이면 해결 가능(미착수).

⚠️ `compute()`에서 **`rate` 산출이 관리코드 해소 뒤로 이동**했다(쿠폰이 상품 속성에 따라 달라짐).
채널 추가·수정 시 이 순서를 깨지 말 것.

## ★ 채널 간 가격 비교 = **세트당가**로 정규화 (2026-08-28)

채널마다 리스팅 묶음 단위가 다르다(1세트·3세트·5세트…). 판매가를 그대로 비교하면 무의미.

```
세트당가 = 판매가 ÷ (N × 박스내품)      # N=hapo 또는 바코드, 박스내품=giftset_lineup_2026
```

코드별로 **최저 세트당가 리스팅**을 대표로 잡고 채널 간 비교한다(소비자가 보는 최저가).

★ **격차의 뿌리는 마진이 아니라 수수료다.** 같은 마진을 지키면
`(1−타채널수수료)/(1−자채널수수료)` 만큼 가격이 벌어진다.
ESM(17.5%) vs 스마트스토어(6%) = **+13.9%**. 2026-08-28 실측 +13.4%로 일치.
→ **고수수료 채널에서 규격품(선물세트)은 구조적으로 불리하다.** 기준마진을 3% 하한까지 밀어도
격차가 8.4%까지밖에 안 좁혀진다. 가격 외 수단(광고·채널 쿠폰)이 필요한 영역.

## ★ 택배비 포함 마진율 — 배송건수는 **EasyAdmin 합포박스키**로 (2026-08-28)

과거 실적의 실질 마진을 볼 때 ERP `판매이익`은 **택배비 차감 전**이다. 택배비를 반영하려면
배송 건수가 필요한데, **ERP 수량은 세트 단위라 배송건수가 아니다**(45-11: 수량 4,989 / 배송 434).

```
택배비 포함 마진율 = (판매이익 − 2700 × 배송건수) ÷ 매출
배송건수 = orders/easyadmin_*.parquet 의 합포박스키 nunique   ← 추정 아님, 실측
```

★ 택배비 부담은 상품마다 크게 다르다 — 세트당 배송 개수에 반비례.
2026-08-28 실측: 45-11 −1.2%p(11.5개/배송) vs 01-19-10 **−6.8%p**(2.9개/배송).
**소량 다건 상품이 택배비에 취약**하므로 마진 판단 시 반드시 구분할 것.

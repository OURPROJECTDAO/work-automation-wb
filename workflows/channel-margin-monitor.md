...
## ★ 기준 초과가 보일 때 — 가격을 내릴지, 기준을 올릴지 (2026-08-28)

`탐지 > 0`(현마진 > 기준마진)이 무더기로 뜨면 처방이 **채널 상황에 따라 정반대**다.
같은 날 ESM과 스마트스토어에서 반대 처방이 나왔다.

| | ESM | 스마트스토어 |
|---|---|---|
| 진도 | 작년의 20% 🔴 | 작년의 99% 🟢 |
| 세트당가 위치 | 전 채널 **최고** | 전 채널 **최저** |
| 처방 | **가격을 기준에 맞춤**(정렬) | **기준을 가격에 맞춤**(상향) |

**판단 규칙**
- 안 팔리고 + 비싸다 → **가격 정렬**(재원 0). 기준마진은 맞는데 가격이 떠 있던 것.
- 잘 팔리고 + 이미 최저가 → **기준 상향**. 기준마진이 낡은 것이지 가격이 틀린 게 아니다.
  여기서 가격을 더 내리면 채널 간 가격 질서가 흔들리고 채널 목표이익이 무너진다.

**기준 상향 시 선정 기준** = *이번 시즌* 실판매 있음 ∩ 최저마진이 기준보다 1%p 초과.
"작년에 잘 팔림"은 근거로 쓰지 않는다(올해 안 팔리면 그 가격이 맞다는 증거가 없다).

★ **제안값은 그 코드 리스팅들의 최저 마진**(최고치 아님). 한 코드에 1세트/박스/무료배송이 섞여
마진이 갈리는데, 최고치로 잡으면 나머지 리스팅이 전부 미달로 뜨며 **가격 인상 신호**가 나온다.
최저치면 가격을 하나도 안 건드려도 전건이 기준 이상. `propose_baseline`이 쓰는 방식과 동일.

## ★ 초과 마진 상위 = 인하 후보가 아니다 — 입력 오류부터 걸러라 (2026-08-28)

권장가가 **비정상적으로 큰 인하**(−20% 이상)를 가리키면 드리프트가 아니라 **데이터 오류**를 의심할 것.

실사례 `01-19-10`: "선물세트 **1개**"(상품번호 …031)가 **51,500원**인데
"선물세트 **4개**"(…032)도 **51,500원** → 1세트 리스팅에 박스 가격이 들어간 **판매가 입력 오류**.
표에는 현마진 11.5% · 권장가 12,100원(**−76.5%**)으로 떴다. N=0.25는 정상이었으므로
**합포 N 오류가 아니다**(2026-08-04 함정과 구분할 것).

**감별법** = 세트당가(`판매가 ÷ (N × 박스내품)`)로 정규화해 **같은 코드의 형제 리스팅과 대조**.
1세트가 박스보다 세트당 비싼 건 정상이지만, 배수로 같으면 가격 입력 오류다.
...
## 기준마진 상향 사례 — 쿠팡 45-21 스팸8호 5% → 8% (2026-09-07)

8/28 스마트스토어 8종 현실화와 **같은 계열 처방**. 판단 흐름 정본화:

1. **진단 먼저** — `compute_listing` 실측으로 그 코드의 전 리스팅 마진을 본다.
   쿠팡 `45-21` 5건이 5.02~5.17%(기준 5.0%)로 **전부 기준선에 붙어 있었다** → 여유 0.02~0.17%p.
   이 상태에서는 **가격 정렬(기준에 맞추기)로 나올 게 없다**. 정렬이 유효한 건 초과 마진이 쌓여 있을 때뿐(ESM 8/31 사례).
2. **잘 팔리는데 기준선에 붙어 있으면 기준이 낡은 것** → 기준을 올린다.
   근거는 "작년에 잘 팔림"이 아니라 **이번 시즌 실판매**여야 한다(8/28 규칙 유지).
3. 인상률 감각 — **마진 3%p 올리는 데 쿠팡 가격은 3%대만 오른다**(수수료 12%·배송비 0 → 정산액=판매가×0.88).
   실택배비 2,700이 순비용이라 저단가 옵션일수록 인상률이 커진다.
4. 타 채널과의 세트당가 격차를 확인한다. `45-21`은 쿠팡 인상 후에도 스마트스토어(10.6%)만 더 높고 나머지 8채널은 5% 유지 →
   **쿠팡만 세트당가가 벌어지므로 며칠 반응 관찰**.

### baseline_margin.csv 단일 셀 수정 검증 6종 (재사용 레시피)
① 셀 단위 diff가 **정확히 의도한 개수**인지 + 예상 밖 0 ② 행수·CRLF·BOM 보존
③ 타 채널 컬럼·타 코드 행 무변형 ④ `compute_listing` **before/after 대조로 변동 리스팅이 해당 코드뿐**인지
⑤ PUT 직전 최신 sha 재취득 ⑥ 사후 원격 재조회 바이트 일치
→ baseline은 **GitHub 라이브 read라 재배포·Reboot 불필요**, 앱 즉시 반영.

### 쿠팡 가격변경 시트(filter형) 검증
`build_filter_price_xlsx` 산출은 항상 ① missing·skipped 0 ② 라운드트립 재파싱 권장가 일치
③ **`inlineStr` 잔존 0 + `sharedStrings.xml` 존재**(쿠팡 업로더 POI 엄격형 호환)를 확인한다.
업로드 후 **'전체 교체'로 재다운로드·갱신** 필수 — raw 네이티브를 유지해야 하므로 '신규만 추가' 금지.


## 선물세트 hapo 누락 — 전 채널 스윕 (2026-09-08)

`50-72-05` S15호가 ESM·캐시노트에서 **1세트 리스팅만 hapo 미등록 → 마진 −428%** 로 떠 있었다
(6세트는 세트수==박스내품이라 N=1 기본값이 맞아 정상). 사용자 지적으로 발견.

**재발 방지 루틴 — 신규 등록 배치 직후 이 스윕을 돌린다.**
```
10개 채널 저장 listing 에 compute_listing 실행
  → 선물세트 유니버스(product_attributes 최종분류 ∪ giftset_lineup) ∩ 마진율 < −50% 스캔
```
2026-09-08 실행 = ESM 2 · 캐시노트 2 · 나머지 8채널 0건.
**−50% 미만은 정상 가격 문제로는 나올 수 없는 값이라 hapo/코드 매칭 오류의 서명**이다.

★ **표기 정밀도** — 박스내품 6의 1세트는 정확값 `0.16666666666666666` 을 쓴다
(형제 `51-31` 이 1/3·1/2·2/3 정확값 사용, `148,500÷6=24,750` 원단위 검산 가능).
스마트스토어는 바코드 칸에 `0.1667` 이 들어가 있어 채널 간 표기가 갈린다(금액 4원 차, 무해).

★ **참조 파일은 헤더를 먼저 찍고 쓴다.** `listing_*.csv` 의 관리코드 컬럼명은 `관리코드`가 아니라
**`코드`**이고, 파일명 키는 `CHANNEL_CONFIG` 의 키(`스마트스토어`)가 아니라 **`cfg["key"]`**(`smartstore`)다.
둘 다 이번에 밟았다.


## 권장가 = 판매가 필드용 값 (2026-09-09 픽스)

`권장가 = ⌈((매입가+실택배비)/(1−기준마진) − 배송비×정산계수)/(1−수수료−쿠폰율) **+ 즉시할인 + 포인트**⌉`

역산이 내놓는 것은 `판매가net` 이고, 채널에 입력하는 것은 **판매가**다. 되더하기가 빠지면
즉시할인만큼 싸게 팔린다. **스마트스토어가 즉시할인 필드를 쓰는 유일한 채널**이며 770건 중 331건이 해당.
2026-09-09 이전 권장가를 그대로 반영해온 스마트스토어 리스팅은 **일괄 재반영이 필요**하다
(인상 필요 44건 · 평균 +2.65% · 건당 730원).

★ 기준마진을 조정한 뒤에는 **권장가의 방향이 기준 방향과 같은지** 확인한다.
"기준을 올렸는데 권장가가 내려간다"면 계산 기준 불일치다.


## ★ listing 스냅샷은 세션 중에도 갱신된다 (2026-09-11)

가격변경 시트를 만들기 직전, 같은 세션 앞부분에서 받아둔 `listing_*.csv` 와 내용이 달랐다.

| 채널 | 세션 초반 | 시트 생성 직전 | `meta.updated_at` |
|---|---|---|---|
| 스마트스토어 | 750행 | 749 | 2026-09-11 15:12 |
| 쿠팡 | 1,203 | 1,177 | 16:08 |
| ESM | 1,287 | 1,250 | 15:20 |
| 캐시노트 | 663 | 651 | 15:21 |

사용자가 세션 진행 중에 상품관리 갱신을 돌린 결과(품절 처리 반영). 오래된 본으로 시트를 만들면
**이미 내려간 리스팅에 가격을 기입**하거나 신규 리스팅을 빠뜨린다.

**규칙**: 챗 네이티브로 가격변경 시트를 만들 때는
① 시트 생성 **직전에** `listing_*.csv`(+ filter/bulk형은 `.xlsx` raw)를 재다운로드하고
② `listing_*.meta.json` 의 `updated_at`·`rows` 를 출력해 남긴다.

2026-06-23 식봄 stale 함정과 같은 계열이나, 그때는 **세션 간** stale이었고 이번은 **세션 내** 변경이다.
"권장가는 현재 저장 listing·기준마진율 스냅샷 기준"(챗 네이티브 절)의 '현재'가 몇 시 기준인지까지 확인해야 한다.

## 채널 선택적 기준마진 인하 — 4채널만 (2026-09-11, `45-07-06` J호)

`baseline_margin.csv` 는 관리코드 1행 × 10채널 컬럼이라 **일부 채널만 바꾸는 것이 기본 동작**이다.
D-14 긴급 지시로 J호를 스마트스토어·ESM·캐시노트·쿠팡 **4개 셀만** 4.5% → 3.0%
(자사몰·배민·식봄·올웨이즈·토마토·알리는 4.5% 유지). 커밋 `292ccc7f`.

§ "baseline_margin.csv 단일 셀 수정 검증 6종" 레시피를 그대로 적용해 전건 통과:
셀 diff 정확히 4개 · 행수 1,358·CRLF·BOM 없음 보존 · 타 채널 컬럼과 나머지 1,357행 무변형 ·
`compute_listing` before/after 대조 시 **권장가 변동이 4채널 통틀어 J호 8건뿐(타 코드 0건)** ·
PUT 직전 sha 재취득 · 사후 재조회 바이트 일치.

실측 인하폭 — 같은 1.5%p 기준 인하인데 채널마다 가격 변화가 다르다:

| 채널 | 수수료(+쿠폰) | 배송비 | 가격 변화 |
|---|---|---|---|
| 스마트스토어 | 6% | 0 / 3,000 | −2.9 ~ −3.2% (즉시할인 900 신설) |
| 쿠팡 | 12% | 0 | −2.1% |
| ESM | 17.5% | 3,000 | −1.5 ~ −1.9% |
| 캐시노트 | 6% + **의무쿠폰 5%** | 3,000 | **−1.7%** (rate = 1−0.06−0.05) |

★ 캐시노트는 쿠폰이 rate에 감산으로 들어가 **기준을 내려도 가격은 가장 덜 내려간다**.
★ 스마트스토어는 즉시할인 필드를 쓰는 유일한 채널이라 `compute_new_prices` 가 **판매가 유지 + 즉시할인 인상**으로 내놓는다
(권장가 = 판매가 − 즉시할인 이 맞는지 검산할 것 — 2026-09-09 되더하기 픽스와 짝).


---

## 4채널 공통 선물세트 교집합 레시피 (2026-09-11)

스스·쿠팡·캐시노트·ESM 공통 판매 선물세트를 뽑을 때:
1. `listing_*.csv` · `product_master.csv` · `giftset_lineup_2026.csv` 를 **매번 새로 받는다**
2. `listing_*.meta.json` 의 `updated_at` 으로 채널별 최신화 시점을 먼저 확인 —
   **한 채널만 며칠 묵어도 교집합이 거짓으로 좁아진다**
3. 채널별 `compute_listing` → `관리코드` 집합 → `set.intersection`
4. **라인업 정본과 교차**해 선물세트만 남긴다(낱상품·비시즌 코드 제거)
5. 코드별 채널별 **최저 마진율** vs `기준마진율` 로 미달 표시

### ★★ 재고는 말할 때마다 마스터를 새로 받는다
2026-09-11, 9/7 스냅샷에서 출고분을 빼 암산한 재고를 사용자에게 말했다가 전부 틀렸다 —
`45-10-09` 439→실제 **-1,043** · `45-07-06` 1,890→실제 **101** · `45-09-01` 45→실제 **147**.
같은 날 오후 12K호는 **557** 로 회복(입고 반영)됐다.
**스냅샷 + 암산 금지.** 재고 수치를 답할 때는 `reference/product_master.csv` 를 그 자리에서 재수신한다.

### 2026-09-11 스냅샷 요지
공통 선물세트 **22종**(라인업 41종 중). 🔴재고 위험 6종 — `45-12-05` **0** · `45-05` **-70** · `50-71-06` 32 ·
`45-08` 76 · `45-09-01` 93 · `45-27` 96 (전부 4채널 동시 노출이라 품절이 네 곳에서 같이 난다).
⚠️기준마진 미달 13건 중 **7건이 ESM**, 참치 3종(`51-30`·`51-30-02`·`51-31`)이 나란히 미달.
`51-31`은 타 3채널 6.7~7.7% vs **ESM 4.0%** 로 격차가 가장 크다.
스스 미달 4건 중 3건은 0.04~0.1%p 차이로 사실상 기준선(실제 갭은 `45-20-05` 4.09% vs 5.0% 하나).

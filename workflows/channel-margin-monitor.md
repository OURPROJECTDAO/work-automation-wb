...
- 상태: **운영중** — 모니터 + 가격 일괄변경(양식 내보내기)·검색·네이티브 다중행 선택 구현·실사용 확인(2026-06-11). 스마트스토어. 타 채널은 레시피로 점진 추가.
...
- `app/pages/6_채널마진모니터.py` (독립 페이지): 채널선택 → 저장 listing 자동로드 → KPI + **검색(상품번호·관리코드·상품명 부분일치)** + 필터(코드유형·마진미달·재고0·제한·미매칭) + **st.dataframe 네이티브 다중행 선택**(헤더 전체선택+개별, `on_select=rerun`·`selection_mode=multi-row`·`event.selection.rows`) + CSV/가격일괄변경 양식 내보내기. 선택은 현재 필터/검색 화면 기준(정렬 시 리셋 — Streamlit 한계). ⚠️ data_editor CheckboxColumn 헤더 전체선택은 신뢰 불가 → st.dataframe 네이티브 사용.
...
- logs/2026-06/2026-06-11-channel-margin-monitor-native-row-selection.md (선택 네이티브 다중행)

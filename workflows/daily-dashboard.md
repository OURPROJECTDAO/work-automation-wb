...
_갱신: 2026-06-22 (이상치/전체 표에 **최근 적재 2개월 정산 매출(net)** 컬럼 2개 — (채널×관리코드)별 월합계로 매출 추세를 사람이 눈으로 판단(화살표/기울기 X·노이즈 0·사용자 결정). ★달력월 고정 X=**실제 적재 최신 2개월**(현 4·5월·6월 미적재 빈칸 함정 회피)·헤더=실월명·자동 롤. 신규 cache `_recent_month_sales`(store.list_partition_months 최신 n개월만·채널=상호명→cc.label 괄호앞·관리코드 _nfc 매칭). 검증=8채널 매칭·점포함코드 매칭(52-55. ESM 10,395→395,904)·24-34 0/0=타채널만(정상). master 5월까지라 6월은 7월초 [데이터추가] 적재 후. 커밋 b7b0d34·page-only·Reboot 불요)_

_갱신: 2026-06-22 (이상치 표 가독성 3건 — ① 금액 컬럼 콤마(NumberColumn format="localized"·선택그리드라 Styler 불가·streamlit≥1.36) ② **월마진% 컬럼**(4월마진%·5월마진% = 정산 총마진율 판매이익÷판매금액·**gross 택배전**·매출0은 — ·`_recent_month_sales` lk값 (매출,판매이익) 튜플로 확장) ③ **데일리 페이지 풀폭**(core/ui.py .block-container max-width:1180px 전역 캡 → 이 페이지만 page-scoped max-width:none 해제·타페이지 1180 유지). ★월마진%=gross라 오늘 net 마진%와 수준 다름(추세용·진짜 net은 orders+ship_alloc 필요·미적용). 커밋 db7eb50·page-only·Reboot 불요)_

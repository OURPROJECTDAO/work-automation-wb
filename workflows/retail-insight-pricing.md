...
★ 리테일 제출본은 **4열**이다 — `바코드 · ERP 상품명(일부 품목 상품명 교체) · 현재가 · 변경가`.
(`등록된_상품_정보`의 9열 `상품코드·상품명·규격·입수·판매단위·면과세·Trade전시여부·품절여부·정상가`은
**리테일이 우리에게 주는 현황 export**이지 제출 양식이 아니다.)
신규 등록분은 **바코드·현재가 공란**(바코드 미발번), 변경가만 채운다.
골든 = `리테일앤인사이트_선물세트_20260825_정리.xlsx`.

## 전용 함정

- ★ **8/25 제출분이 6일째 미반영된 전례**(2026-08-31 발견). 신규 7종을 제출했으나 리테일 등록 파일에 없었다.
  → **제출 후 다음 export에서 반영 여부를 반드시 대조할 것.** 시즌 중엔 손실이 크다.
- ★ **기존 리테일가는 공식과 안 맞는다**(2026-08-31: 28종 중 6종만 재현). 실제가가 공식가보다 높고
  (45-08 +15,780·45-10-01 +11,390) 매입가 변동을 안 따라간 낡은 가격으로 보인다.
  **기존가를 기준으로 신규 가격을 잡지 말 것** — 신규는 공식대로 간다.
- `retail_barcode_map` 미매핑 건이 있다(2026-08-31 기준 17건). 그중 **바코드 자리에 `신규바코드필요`
  텍스트가 들어간 행**도 있으므로 숫자 바코드만 가정하지 말 것.

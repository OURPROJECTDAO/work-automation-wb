# 로드맵 / 백로그

## 완료
- KB 기억 시스템 구축. 2026-05-31. (decisions/0001)
- 아키텍처 확정: Streamlit + Python + Community Cloud. 2026-06-01. (decisions/0002)
- **Phase 1 완료**: 오픈마켓합포도서산간확인V7 Python 재구현 + pytest PASSED. 2026-06-01.
- **Phase 2 완료**: Streamlit 앱 배포 + 기준 데이터 관리 UI. 2026-06-01.
- **Phase 3 2종 완료**:
  - 온누리양식_발주서 (onnuri_order.py). 2026-06-01.
  - 발주서출력업무 (logistics_order.py) — 2-phase+게이트, 프린트 디자인, 골든 테스트. 2026-06-02. (decisions/0003)

## 다음 (우선순위)
1. ~~Phase 1~~ ✅
2. ~~Phase 2~~ ✅
3. **Phase 3 계속**: 나머지 템플릿 1종씩 Python 이관.
   - 온누리양식_발주서 ✅
   - 발주서출력업무 ✅
   - 다음 템플릿: 사용자 안내 필요.
4. Phase 4: 대시보드 페이지 (매출/현황 데이터 뷰).

## 나중
- 입력 자동 수집 (API/크롤링).
- 동료 접근 확대 / 필요 시 Railway 이전.
- **KB 구조 확장성 아이디에이션**: 워크플로우 20개+ 대응. pitfalls.md·state.md 단일 파일 비대화 문제. workflows/ 디렉토리 분리 등 구조 개선안 검토 필요.

_갱신: 2026-06-02 (발주서출력업무 완료 반영)_

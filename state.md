...
## 다음 한 수
- **★★ 기준마진율 최적화(두뇌④) 설계확정·미구현 (2026-06-19, ADR 0026).** P×C마다 최적 기준마진율을 데이터로 잡아 cmm 편집창 prefill. 베이스=순이익누적85% proven채널 순이익가중평균·볼륨×(마진vs베이스) 4분면(저볼륨고마진=절반스텝 인하/저볼륨저마진=hold-low)·탄력성=채널내부비교·회전 markdown·채널독립·전부 사람저장(🟢🟡🔴)·관망=2회 무반응 park(영구아님)·신규 항상 실험. 라벨: 자동(4분면·채널편차·회전)+사람(매출목표·시즌), 보류(시장가=ESM쿠폰왜곡·재구매/미끼=고객데이터선결), 제외(원가추세→대시보드). 데이터 토대 전부 보유(매출자료·ship_alloc·orders·두뇌②·nadl매핑). **다음=프로토타입**(권장값 산출→표→prefill, 매칭·고볼륨 상품부터). 정본 workflows/margin-optimizer.md·로그 2026-06-19-margin-optimizer-design.md.

...
| dashboard (영업이익현황 대시보드) | 4 | 운영(최소) · 매출 KPI+구분/거래처/상품/관리코드별 집계 · 3개년 313K행 · 차트/물류량/업로더 점진추가 | workflows/dashboard.md |
...
- Phase 4 (운영 최소): 대시보드 — 데이터계층+저장어댑터+3개년 적재(313K행)+최소 페이지(매출 집계) 배포. 차트/물류량/업로더 점진추가. 2026-06-08. (decisions/0006)

## 막힌 것 / 이슈
- 없음. (B2 인프라 완료 — repo·PAT R/W·st.secrets 검증됨 2026-06-08.)

## 다음 한 수
- **Phase 4 대시보드 점진 확장**: 최소 페이지(매출 집계) 배포 완료. 다음 후보 — ① 멀티연도 월별 매출 추이 차트 ② 이익률 KPI ③ 물류량(수량÷박스내품) ④ 이익/물류량 콤보 ⑤ 거래처 그룹 관리 탭 ⑥ 증분 업로더 UI. 사용자 우선순위 따라 1개씩. 상세 workflows/dashboard.md.
...
_갱신: 2026-06-08 (Phase 4 대시보드 최소버전 배포 — 매출 집계, 이후 점진 확장)_

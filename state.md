...
- **★★ 재개점 (2026-06-15): 주문 백필 완료 ✅(2023-03~2026-05·398,873행·2023 여름 갭만). 데이터현황 페이지 1단계 ✅(통합 데이터 관리·적재 범위/갭·가격이력 포함·x축 YYYY-MM). **2단계 업로드=나중에 일괄**(매입현황 등 적재 대상 다 갖춘 뒤·사용자 확정). **두뇌① 강화 완료(3렌즈+velocity, 2026-06-15)** → 다음=**두뇌② 입고·품절 예측**(현재고+velocity+리드타임1c). →두뇌③. 로드맵 정본=systemmap.json. ⚠️ 두뇌① core 수정 → Reboot app 1회.**
  ...
  - **확정 순서(2026-06-15)**: ① **P2 송장 실배분→진짜 마진** ✅완료(core ship_alloc.py·대시보드 추정송장·k 교체) → ② **주문 백필 ✅완료**(과거 통파일 .xlsx 2023-03~2025-06 + 분할 .xls = 2023-03~2026-05·398,873행) → ③ **두뇌③ 채널 가격 A/B**(신규 페이지). **velocity 가중=후순위(지금 불요).** 상세 우선순위 정본=systemmap.json intelligence-layer.roadmap.

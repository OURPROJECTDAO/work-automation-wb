...
| sikbom-register (식봄 일괄등록) | — | 운영준비 (1차 74건 양식 생성·검증·실업로드 대기) | workflows/sikbom-register.md |
...
## 다음 한 수
- **★ 식봄 일괄등록 셋업·1차 74건(2026-06-18) — 양식 생성·검증 완료·실업로드 대기.** 식봄 최초 상품등록(마진모니터/송장/행사기획엔 기존). 신규 워크플로우 sikbom-register + reference/sikbom_category.csv(266·2축매트릭스 평탄화·사용자 큐레이션). 양식=상품_대량등록(29열·r8부터·상품 카테고리 시트 보존)·수수료7%·마진=스마트스토어 baseline(없으면10%)·판매가 공통식(2500/내림)·**배송비명 L=N==1?'유료배송(수량별)':'수량별 배송비 N'**(N=박스합포/낱개합포×박스내품·식봄센터 사전등록 의존)·카테고리=명 그대로(B/C/D)·이미지=양식外 별도(상품코드 매칭·**phase2**). 입력=업로드감시 3채널출력(합포단위·면세과세 컬럼·식봄'업로드필요' 필터). 비고 19건 검색/이미지 확정(비스더블랙=캔커피·맛기름=향미유 참기름·피죤=섬유유연제2입 등). **다음=실업로드 검증→(성공시)이미지 단계.** ⚠️openpyxl 저장본 식봄 첫 검증(거부 시 쿠팡식 zip수술). 산출 식봄_일괄등록_74건.xlsx·검토본 식봄등록_검토_74건.xlsx. 상세 workflows/sikbom-register.md·로그 2026-06-18-sikbom-register-setup.md.

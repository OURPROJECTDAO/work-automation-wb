...
- **기준마진율 편집(현재 마진율→기준) 추가 완료(2026-06-12, ADR 0016)** — 전 채널 공통. 모니터 표 선택→'기준마진율=현재' 버튼(offset 그대로/−1%p)·충돌 시 관리코드별 사용자선택·0.1%p반올림. 그 채널 baseline_col 컬럼만 수정(타채널 보존). **baseline을 GitHub 라이브read(compute_listing override)로 바꿔 저장 즉시 반영**(재배포 불요). 미달 상품 목표 인정→미달 해제(검증: 미달 10→0, 전체 49→29). 향후 ②인라인 ③일괄. ⚠️ core+page → Reboot.

...
## 후속 (같은 세션 · 사용자 피드백)
사용자: "추가 매출통계 입력칸은 불필요(제이티·리테일도 그냥 발주자료로 올림). 단, 배민·스스가 제이티·리테일엔 필요 없는데 그게 없으면 실행이 안 된다."
→ 두 가지 조치:
1. **배민·스스주문 선택화** — `open_baemin(None)/open_sss(None)→None`, `process_baemin/process_smartstore(None)→{}`, `run()` baemin/sss 기본 None. 페이지 실행 조건 `if f_baeju:`(발주자료만 필수). 미업로드 시 스마트스토어전체·배민상회전체 G=선결제비 폴백(검증: G 500/700 폴백·제이티/리테일 정상 분배). 회귀 `test_optional_shipping_none`.
2. **④ 추가 매출통계 입력칸 제거** — 코어 `parse_sales_stats`/`run(stats_files=)`는 휴면 보존(필요 시 재노출). page에서 ④ 업로더+병합 표시 삭제, run 호출 정리.
- 커밋: core 17b95d9 · page a00ff58 · test 7c3edef. ast 3파일 OK. **⚠️ core 변경 → 재배포 후 Reboot 1회**.
- 최종 사용 흐름: 발주자료만 올려도 실행 → 제이티/리테일(발주자료에 섞임) 포함 분배. 배민·스스는 전체 정산 때만.
